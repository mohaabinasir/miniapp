import { createContext, useContext, useState, ReactNode, ElementType, useRef, useEffect } from "react";
...
interface PopupButton {
  ...
  onClick?: (data: { inputs: { value: string; setIcon: (Ico: ElementType) => void }[]; checkboxes: boolean[]; input: { value: string; setIcon: (Ico: ElementType) => void }, close: () => void}) => void;
  [key: string]: any;
}
...
interface PopupOptions { title?: string; text?: string; inputs?: PopupInput[]; checkboxes?: PopupCheckbox[]; buttons?: PopupButton[]; radio?: PopupRadio, onClose?: (cb: any) => void }
...
const setIcon = (Ico: ElementType, idx) => {
      setOptions(prev => {
...
useEffect(() => {
    if (!options || !options.inputs || hasFocused.current) return;
    setTimeout(() => {
...
return ( <PopupContext.Provider value={{ popup }}> {children} {options && ( <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" style={{
    ...
    <div className="bg-neutral-900 rounded-2xl p-5 bt-3 w-[70%] space-y-4" style={{
        ...
        {options.title && <h2 className="text-xl font-semibold text-[#007bff]">{options.title}</h2>}
        {options.text && <p className="text-neutral-300 text-sm mt-2">{options.text}</p>}
        </div>
...
{options.inputs && (
          <div className="space-y-3">
            ...
            return(
             ...
             <div className="relative" key={i}>
              ...
              />
              {inp.Icon && <span className="absolute top-1/2 right-0 -translate-y-1/2">{Icon}</span>}
              </div>
            )})}
        ...
        <div className="flex justify-end gap-1 pt-1 w-full">
          {options.buttons?.map((b, i) => {
            ...
            <button
              key={i}
              className="px-4 py-2 rounded-md bg-transparent text-white text-sm ripple w-12"
              style={{color}}
              data-rcl={color + '40'}
              onClick={() => {
