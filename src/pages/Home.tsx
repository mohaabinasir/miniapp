...
import {ArrowLeft, EllipsisVertical, LayoutGrid, X} from 'lucide-react'
...
async function startAccelerometer(refreshRate = 60) {
  if ('Accelerometer' in window) {
    const sensor = new Accelerometer({ frequency: refreshRate })
    sensor.addEventListener('reading', () => {
      console.log(`X: ${sensor.x.toFixed(2)}, Y: ${sensor.y.toFixed(2)}, Z: ${sensor.z.toFixed(2)}`)
    })
    sensor.start()
...
export default function Home() {
   ...
   const setAm = (rr: boolean | number = 1000) => {
    ...
    useEffect(() => {
        window.addEventListener('message', async(event) => {
             ...
             if (eventType === 'web_app_set_header_color') {
                 const color = eventData.color || themeParams[eventData.color_key] || '#181819';
             ...
             if (eventType === 'web_app_open_popup') {
                 const _btns = eventData.buttons.map((b) => {
                     b.onClick = () => {
             ...
             if (eventType === 'web_app_request_fullscreen') {
                 try {
                     ...
                     popup({text: 'This mini app needs to open in full screen', title: 'Fullscreen request', buttons: [
             ...
             if (eventType === 'web_app_biometry_request_auth') {
                 if (await authenticate(eventData.reason)) {
             ...
             if (eventType === 'web_app_invoke_custom_method') {
                 ...
                 try {
                     const cs = storage.get('cloud_storage') || {};
                     const result: any = {};
                     eventData.params.keys.forEach((v) => {
             ...
             if (eventType === 'web_app_toggle_orientation_lock') {
                 if (eventData.locked) {
                     setDetectOrientation(true);
                     setLockOr({lock: true, landscape: window.matchMedia('(orientation: landscape)').matches});
                 };
             }
        })
    }, [])
    ...
    return(<div className="w-screen h-screen text-white" style={{
        background: theme.bg_color || themeParams.bg_color
    }} ref={container}>
        
        <UrlPopup open={open} setOpen={setOpen} onSubmit={() => setOpen(false)} onSubmit={(vl: string) => {
            setUrl(vl);
            storage.setUrl(vl);
            storage.add(vl);
            setOpen(false);
        ...
        {!closed && <header className="w-full p-2 sticky top-0 left-0 flex justify-between border-b-1 border-[#333]" style={{
            ...
            }} className="ripple p-1.5 rounded-full">{backBtn.is_visible ? <ArrowLeft /> : <X />}</button>
            <span className="font-bold">MiniApp</span></span>
        ...
        {webUrl && !closed && <iframe src={webUrl} className="w-full h-full border-none" onError={() => setFailed(true)} ref={webapp}>
        ...
        {closed && <div className="flex w-full h-full justify-center items-center flex-col gap-2 overflow-hidden">
        ...
        {(mainBtn.is_visible || secBtn.is_visible) && <div className={`w-full bg-black py-2 px-3 sticky bottom-0 left-0 flex justify-center gap-2 ${['top', 'bottom'].includes(secBtn.position) ? 'flex-col' : ''}`}>
            ...
            {!closed && btns().map((btn, ind) => {
                ...
                <span>{btn.is_progress_visible ? <CircularProgress size={24} color="string"/> : btn.text || "Continue" }</span>
            </button>)
            })}
